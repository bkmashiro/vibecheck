...
VibeCheckはgitコミット履歴にある確認可能なAI支援シグナル—明示的なAI帰属、同一作者のコミットバースト、修正チェーン、高密度セッション—を分析します。
...
Vibe Index v2はGitHubの直近100コミットの**確認可能なメタデータ**を使い、**0〜100**に正規化した指数とサンプル信頼度を別々に表示します。ソースコード、プロンプト、ローカルのエージェントログは読みません。

| シグナル | 証拠ポイント | カテゴリ上限 |
|---|---:|---:|
| 明示的なAI帰属 | +18/コミット | 45 |
| 同一作者のバースト速度 | +3 / +5 / +8 | 25 |
| 同一作者の高速連続コミット | +4 | 10 |
| 同一作者の修正チェーン | +5 | 10 |
| 高密度セッション | +4 / +8 | 20 |

マージコミットを除外し、人間同士の通常の `Co-Authored-By:` はAI扱いせず、総行数だけでは加点しません。作者の断定やコード品質評価ではなく、監査可能な比較用ヒューリスティックです。
...
問題はAIを使うことではなく、*生成*と*理解*の比率です。VibeCheckの高スコアはスピードと相関しており、理解とは相関していません。85点のリポジトリは、AIが生成した全行を深く理解した作者による技術的傑作かもしれない。または、作者がデバッグできないAI出力の山かもしれない。
